...
任务五把 CYP3A4、CYP2D6、CYP2C9 的可逆抑制与酶失活/再合成，接入仓库中**实际的 Task 4 七状态药物分布模型**。六个药名用于情景定位；主要 PK、Ki、KI、kinact 数值均为明确列出的假设，不能用于临床给药或停药决策。
...
```sh
python task5_cyp_ddi/run_task5_cyp_ddi_mechanism_based_inhibition.py --self-test
python task5_cyp_ddi/run_task5_cyp_ddi_mechanism_based_inhibition.py --config task5_cyp_ddi/compound_panel_example.json --out work/task5_reproduction
python task5_cyp_ddi/run_task5_cyp_ddi_mechanism_based_inhibition.py --write-example work/editable_panel.json
```
