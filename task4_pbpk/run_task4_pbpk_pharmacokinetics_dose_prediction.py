...
Run: python run_task4_pbpk_pharmacokinetics_dose_prediction.py --out .
Export inputs: --write-example compound_example.json
Use inputs: --config compound_example.json --out results
...
运行 `python run_task4_pbpk_pharmacokinetics_dose_prediction.py --self-test --out results` 可复现交付。使用 `--write-example compound.json` 导出模板，编辑后通过 `--config compound.json` 读取。verification.json 保存检查结果，manifest.json 保存版本和文件校验值，四张图均为 300 DPI。脚本自包含，可重新生成所有表格和中英文报告；Task 1 描述符可通过 JSON 模板导入，但不能替代蛋白结合、清除率或毒理学实测参数。
