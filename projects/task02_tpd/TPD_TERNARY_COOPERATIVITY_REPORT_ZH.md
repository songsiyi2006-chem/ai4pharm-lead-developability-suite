...
从仓库根目录执行 `python projects/task02_tpd/run_task2_tpd_ternary_cooperativity.py --output-dir work/task02_reproduction`，即可在指定输出目录再生四张 300 DPI 图、双语报告、CSV 数据、SDF 构象、校验文件、依赖版本、校验和与日志。全部逻辑包含在单个脚本内，不需要网络或外部模板；参数见 `--help`。固定随机种子并单线程构象采样，同一环境下可重复运行；不同 RDKit 版本可能改变构象结果。版本及文件哈希保存在 `manifest_task2.json`。
